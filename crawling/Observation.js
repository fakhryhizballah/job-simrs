...
async function kirimObservation(date) {
    ...
    for (let x of encounters) {
        console.log(x.id)
        let findObservation = await Observation.find({
        ...
        if (findObservation.length === 0) {
            ...
            if (x.class.code === 'IMP') {
                ...
                // console.log(JSON.stringify(bundel));
                ...
                for (let y of kirimObservation.entry) {
                    console.log(y.response.resourceType)
                    console.log(y.response.resourceID)
                    let etag = y.response.etag.split('"')[1];
                    if (y.response.resourceType === 'Observation') {
                        let findObservationSatuSehat = await fetchSatusehat('GET', `Observation/${y.response.resourceID}/_history/${etag}`)
                        await Observation.create(findObservationSatuSehat)
                    }
                    if (y.response.resourceType === 'CarePlan') {
                        let findCareplan = await fetchSatusehat('GET', `CarePlan/${y.response.resourceID}/_history/${etag}`)
                        await Careplan.create(findCareplan)
                    }
